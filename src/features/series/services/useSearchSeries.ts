...
import { USE_SEARCH_SERIES_QUERY_KEY } from "@/shared/services/keys";
...
const queryFn = async (pageParam: number, term: string) => {
  const response = await http.get<TmdbPagination<TmdbItem>>(
    `/search/tv?api_key=${process.env.EXPO_PUBLIC_TMDB_API_KEY}&page=${pageParam}&query=${term}`,
  );
  return response.data;
};

const useSearchSeries = (term: string) => {
  const { data, fetchNextPage, hasNextPage, refetch, isFetching } =
    useInfiniteQuery<TmdbPagination<TmdbItem>, Error>({
      ...
      queryKey: [USE_SEARCH_SERIES_QUERY_KEY, term],
